...
data2=( `cat /var/log/xray/access.log | tail -n 500 | grep -oP 'from \K[0-9.]+' | sort | uniq`);
for ip in "${data2[@]}"
do
jum=$(cat /var/log/xray/access.log | grep -w "$akun" | tail -n 500 | grep -oP 'from \K[0-9.]+' | grep -w "$ip" | sort | uniq)
if [[ "$jum" = "$ip" ]]; then
